...
function formatFileDiffSummary(diffs: any[]): string {
  ...
  for (const diff of diffs) {
    ...
    lines.push(
      `  ${dim}${file}${reset} ${green}+${adds}${reset} ${red}-${dels}${reset}`,
    );
  }
  return lines.join("\n");
}
...
/**
 * Parse a leading slash command from prompt text, mimicking the TUI's
 * command detection (first word starting with / = command, rest = arguments).
 */
function parseSlashCommand(
  ...
  const commandLine =
    firstNewline === -1 ? trimmed : trimmed.substring(0, firstNewline);
  const commandName = commandLine.slice(1).trim();

  if (!commandName || commandName.includes(" ")) return null;

  const args =
    firstNewline === -1 ? "" : trimmed.substring(firstNewline + 1).trim();
...
export class OpenCodeManager {
      ...
      for (const agent of agents) {
        if (agent.model) {
          ...
          if (!provider) {
            logger.warn(
              {
                ...
                providerID,
                modelID,
                availableProviders: providers.providers.map((p) => ({
                  id: p.id,
                  name: p.name,
                })),
              },
            ...
            console.log(
              `\x1b[33m⚠️ Config Warning: Agent "${agent.name}" uses unknown provider "${providerID}"\x1b[0m`,
            );
            continue;
          }
        }
  ...
  async startTask(
    ...
    logger.info(
      { issueNumber: issue.number, sessionId: session.id },
    ...
    try {
      ...
      console.log(
        `\x1b[31m❌ Failed to send prompt: ${error.message || error}\x1b[0m`,
      );
      throw error;
    }
  ...
  async continueTask(
    ...
    logger.info(
      { issueNumber: issue.number, sessionId: previousSessionId },
      "Continuing task",
    );
    ...
    try {
      ...
      logger.error(
        { issueNumber: issue.number, sessionId: previousSessionId, error },
        "Previous session not found, creating new one",
      );
      ...
      return this.startTask(
        issue,
        worktreePath,
        `ai/issue-${issue.number}-recovery`,
      );
    }
  ...
  private subscribeToEvents(task: OpenCodeTaskStatus): void {
    ...
    task.eventSource = this.client.streamEvents(
      ...
      (error: Error) => {
        ...
        console.log(
          `\x1b[33m⚠️ Event stream error for issue #${task.issueNumber}: ${error.message}\x1b[0m`,
        );
    ...
    try {
      ...
      if (sessionStatus) {
        ...
        if (
          ...
          console.log(
            `\n⚠️ Session Status Check [${sessionStatus.state}]:`,
            sessionStatus,
          );
        }
      ...
      logger.error(
        { issueNumber: task.issueNumber, error },
  ...
  private handleEvent(task: OpenCodeTaskStatus, event: OpenCodeEvent): void {
    ...
    if (eventType === "message.updated") {
      ...
      if (
        summary?.diffs &&
        Array.isArray(summary.diffs) &&
        summary.diffs.length > 0
      ) {
        const totalAdds =
          summary.additions ||
          summary.diffs.reduce(
    ...
    if (eventType === "message.part.updated" || eventType === "message.chunk") {
      ...
      if (part?.type === "text" && part.text) {
        ...
        if (task.currentMessageBuffer) {
          logger.info(
            { issueNumber, text: task.currentMessageBuffer },
            "OpenCode message",
          );
      ...
      console.log(
        `\x1b[31m❌ OpenCode Error: ${typeof errorData === "object" ? JSON.stringify(errorData) : errorData}\x1b[0m`,
      );
      ...
      const message =
        popupData.message || popupData.text || popupData.body || "";
      ...
      const hasChanged = !lastPopup || lastPopup.message !== normalizedMessage;
      ...
      if (idleSessionId === task.sessionId) {
        ...
        if (sessionDuration < IMMEDIATE_FAILURE_MS) {
          ...
          console.log(
            `\x1b[31m❌ Session failed immediately (${sessionDuration}ms) - likely config error\x1b[0m`,
          );
        ...
        const hasEmptyModelID =
          task.currentMessageBuffer?.includes("glm-4.7-free") || false;
        if (hasEmptyModelID && sessionDuration < IMMEDIATE_FAILURE_MS) {
          console.log(
            `\x1b[31m❌ Model config error: glm-4.7-free provider has empty modelID\x1b[0m`,
          );
        }
        ...
        if (sessionDuration < IMMEDIATE_FAILURE_MS && !task.errorHandled) {
          ...
          this.handleSessionError(
            ...
            logger.error(
              { issueNumber, error: err },
              "Failed to handle immediate failure error",
            );
          });
        }
        ...
        if (event.error || sessionDuration < IMMEDIATE_FAILURE_MS) {
          logger.info(
            { issueNumber, sessionId: task.sessionId },
            "OpenCode session ended (Failed)",
          );
        } else {
          logger.info(
            { issueNumber, sessionId: task.sessionId },
            "OpenCode session idle - Task Complete",
          );
        }
      ...
      logger.error(
        {
          issueNumber,
          sessionId:
            event.sessionID || event.properties?.sessionID || task.sessionId,
          errorType,
          errorMessage,
          statusCode,
          fullError: error,
      ...
      if (!task.errorHandled) {
        task.errorHandled = true;
        this.handleSessionError(issueNumber, error, task.shareLink).catch(
          (err) => {
            logger.error(
              { issueNumber, error: err },
              "Failed to handle session error",
      ...
      if (error || state === "error" || state === "blocked") {
        ...
        if (!task.errorHandled) {
          ...
          console.log(
            `\x1b[33m⚠️ Session ${eventType}: state=${state}${error ? ", error=" + (error.message || error.name || "unknown") : ""}\x1b[0m`,
          );
      ...
      const inProgress = todos.filter(
        (t: any) => t.status === "in_progress" || t.status === "working",
      );
      const pending = todos.filter(
        (t: any) => t.status === "pending" || t.status === "todo",
      );
      ...
      const shareLink =
        event.shareLink ||
        event.url ||
        event.properties?.shareLink ||
        event.properties?.url;
      if (
        shareLink &&
        typeof shareLink === "string" &&
        shareLink.includes("opncd.ai") &&
        !task.shareLink
      ) {
      ...
      if (eventType !== "message.updated") {
        ...
        if (
          ...
          console.log(
            `\x1b[33m⚠️ Event [${eventType}] may contain error info\x1b[0m`,
          );
        }
      }
    }
  ...
  private async handleSessionError(
    ...
    try {
      logger.info(
        { issueNumber },
        "Reverting issue to ai-task label after session error",
      );
      ...
      try {
        ...
        logger.warn(
          { issueNumber, error: err },
          "Failed to remove ai-in-progress label",
        );
      }
  ...
  async killTask(issueNumber: number): Promise<void> {
    ...
    if (task) {
      ...
      try {
        ...
        logger.error(
          { issueNumber, error },
          "Failed to abort session on server",
        );
      }
